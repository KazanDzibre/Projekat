...
namespace ServerApp
{
    class ThreadFunctions
    {
        ...
        public static void threadFun(AdamCNT AdamComponent)
        {
            ...
            Switch_on.State = "ON";
            Switch_on.Time_start = DateTime.Now.ToString("H:mm:ss");
            Switch_off.Time_start = DateTime.Now.ToString("H:mm:ss");
            ...
            while (true)
            {
                if (AdamComponent.switchRead() == 1 && trenutna == true)
                {
                    ...
                    Switch_on.Time_start = DateTime.Now.ToString("H:mm:ss");
                    Switch_off.Time_end = DateTime.Now.ToString("H:mm:ss");
                ...
                else if (AdamComponent.switchRead() == 0 && trenutna == false)
                {
                    ...
                    Switch_on.Time_end = DateTime.Now.ToString("H:mm:ss");
                    Switch_off.Time_start = DateTime.Now.ToString("H:mm:ss");
                    FileIO.outputSwitch(switchOutList, Switch_on);
                    Console.WriteLine("Usao na promeni OFF");
                    trenutna = true;
                }
